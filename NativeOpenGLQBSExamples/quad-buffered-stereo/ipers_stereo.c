...
draw(void)
{
 static int count=0;
  ...
  fr=gettime();
  sprintf_s(frbuf, 200 ,"Frame rate: %0.2f   LOD: %d   Tot. poly.: %d   Poly/sec: %.1f",
...
#pragma region PIXELFORMATDESCRIPTORREGION 
  ...
  //bool StereoIsAvailable = false;

  //int iPixelFormat = DescribePixelFormat(hdc, 1, sizeof(PIXELFORMATDESCRIPTOR), &pfd);
  //while (iPixelFormat) {
  ...
  //printf("Stereo available : " + StereoIsAvailable);
